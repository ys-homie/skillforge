...
- 交互说明：[动画/手势/状态切换]
- 布局约束表：[组件 → 横向行为 / 纵向行为 / 溢出策略 / 固定尺寸依据]
```

## 布局约束解析（必须输出）

不要把 Figma Frame 的绝对宽高直接翻译成代码中的固定宽高。对每个用户可感知组件标记：

- 横向行为：填满可用宽度、内容撑开、最小/最大宽约束、局部固定宽度。
- 纵向行为：内容撑开、占满剩余空间、最小/最大高约束、局部固定高度。
- 溢出策略：换行、弹性压缩、滚动、裁剪或设计明确要求的单行缩放。
- 固定依据：只有图标/SVG、头像、插画、缩略图、拖拽条、角标、明确尺寸的小按钮等局部元素，或设计/业务明确不允许变化的尺寸，才标记固定宽高。

小型 SVG/图标按钮必须分别提取“外层按钮/交互区域尺寸”和“内层图标视觉尺寸”。主体卡片、弹窗、面板和文案区默认宽度受父约束、高度由内容撑开。

页面包含动态 Banner、长文案、底部固定操作区或空态居中时，额外标注剩余空间和遮挡关系；不得用基准屏幕高度减固定常量作为实现规格。
...
5. **响应式行为** — 分别判断横向、纵向和溢出策略；区分局部固定元素与自适应主体
6. **约束边界** — 记录窄屏、短屏、最长多语言、动态内容和安全区下的表现
